...
<pre class="p-1 text-white">Selected:<b>{{ERselected}}</b><br>Hovered:<b>{{ERhovered}}</b></pre><button class="btn btn-sm btn-outline-{{ERreadonly ? 'danger' :'success'}}"
  (click)="ERreadonly=!ERreadonly" appRippleEffect>{{ ERreadonly ? "Readonly" :"Editable"}} </button>
`,
...
<pre class="p-1 text-white">Model:<b>{{ ctrl.value}}</b></pre><button class="btn btn-sm btn-outline-{{ ctrl.disabled ? 'danger' :'success'}} mr-2"
  (click)="toggle()" appRippleEffect>{{ ctrl.disabled ? "control disabled" :" control enabled"}} </button><button class="btn btn-sm btn-outline-primary mr-2" (click)="ctrl.setValue(null)">Clear</button>
`,
  ts: `
...
<ng-template #t let-fill="fill"><span class="custom-star" [class.custom-full]="fill===100"><span class="custom-half" [style.width.%]="fill">&#9733;</span>&#9733; </span></ng-template>
  ...
  <hr>
  <pre class="p-1 text-white">Rate:<b>{{decimalCurrentRate}}</b></pre><button class="btn btn-sm btn-outline-primary mr-2" (click)="decimalCurrentRate=1.35" appRippleEffect>1.35</button><button
    class="btn btn-sm btn-outline-primary mr-2" (click)="decimalCurrentRate=4.50" appRippleEffect>4.50</button>
  `,
